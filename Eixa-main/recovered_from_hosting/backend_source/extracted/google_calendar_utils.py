...
class GoogleCalendarUtils:
    def __init__(self):
        ...
        if not all([self.client_id, self.client_secret, self.redirect_uri, self.frontend_url]):
            CALENDAR_UTILS_LOGGER.debug("Google Calendar OAuth não configurado - funcionando sem integração de calendário.")
    ...
    async def get_auth_url(self, user_id: str, account_label: str | None = None) -> str | None:
        if not self.oauth_config_ready:
            CALENDAR_UTILS_LOGGER.error(f"OAuth config not ready for user {user_id}. Missing env vars: CLIENT_ID={bool(self.client_id)}, CLIENT_SECRET={bool(self.client_secret)}, REDIRECT_URI={bool(self.redirect_uri)}, FRONTEND_URL={bool(self.frontend_url)}")
            return None
