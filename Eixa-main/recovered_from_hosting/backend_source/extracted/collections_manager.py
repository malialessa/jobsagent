...
def get_user_subcollection(user_id: str, logical_subcollection_name: str) -> firestore.CollectionReference:
    ...
    full_path = f"{user_doc_ref.path}/{real_name}"
    logger.debug(
        "COLLECTIONS_MANAGER | Getting subcollection '%s' as '%s' under user document '%s'. Full path: %s",
        logical_subcollection_name,
        real_name,
        user_id,
        full_path,
    )
...
def get_unscheduled_tasks_collection(user_id: str) -> firestore.CollectionReference:
    ...
    logger.debug(
        "COLLECTIONS_MANAGER | Getting unscheduled tasks collection for user '%s'. Full path: %s/%s",
        user_id,
        get_user_doc_ref(user_id).path,
        collection_ref.id,
    )
