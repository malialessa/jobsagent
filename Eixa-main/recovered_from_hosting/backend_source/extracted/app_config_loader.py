...
def get_eixa_templates():
    ...
    """
    # Verifica se TODOS os templates estão no cache (não apenas um)
    if ("eixa_persona_template_text" not in _config_cache or 
        "user_profile_template_content" not in _config_cache or 
        "user_flags_template_content" not in _config_cache):
