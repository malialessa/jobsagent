...
async def get_sabotage_patterns(user_id: str, n: int = 20, user_profile: Dict[str, Any] = None) -> dict:
    ...
    if detected:
        ...
        # Salvar padrões detectados se houver 3+ ocorrências de qualquer pattern
        total_occurrences = sum(detected.values())
        if total_occurrences >= 3:
            await save_sabotage_pattern_detection(user_id, detected)

    return detected

async def save_sabotage_pattern_detection(user_id: str, patterns: Dict[str, int]) -> None:
    """
    Salva detecção de padrões de sabotagem no Firestore.
    
    Args:
        user_id: ID do usuário
        patterns: Dicionário com padrões detectados e contagens
    """
    client_timestamp = datetime.now(timezone.utc)
    doc_id = f"{user_id}_{client_timestamp.isoformat().replace(':', '-').replace('.', '_')}"
    ...
    pattern_data = {
        ...
        "timestamp": firestore.SERVER_TIMESTAMP,
        "patterns": patterns,
        "top_patterns": [{"phrase": phrase, "count": count} for phrase, count in top_patterns],
    ...
    try:
        sabotage_collection = get_top_level_collection('sabotage_patterns')
        db = _initialize_firestore_client_instance()
        await asyncio.to_thread(db.collection(sabotage_collection.id).document(doc_id).set, pattern_data)
        logger.info(f"Padrões de sabotagem salvos para usuário '{user_id}'. Total: {total_count}, Doc ID: {doc_id}")
    except Exception as e:
        logger.error(f"Erro ao salvar padrões de sabotagem para usuário '{user_id}': {e}", exc_info=True)

async def get_recent_sabotage_detections(user_id: str, days: int = 7, limit: int = 5) -> list[dict]:
    """
    Recupera detecções recentes de padrões de sabotagem.
    
    Args:
        user_id: ID do usuário
        days: Número de dias para buscar
        limit: Número máximo de detecções a retornar
    
    Returns:
        Lista de dicionários com dados de detecções
    """
    db = _initialize_firestore_client_instance()
    detections = []
    ...
    try:
        ...
        sabotage_ref = get_top_level_collection('sabotage_patterns')
        
        query = sabotage_ref.where("user_id", "==", user_id) \
            .order_by("timestamp", direction=firestore.Query.DESCENDING) \
            .limit(limit)
        ...
        for doc in docs:
            ...
            # Filtrar por data se timestamp estiver disponível
            if detection.get('created_at'):
                created_at = datetime.fromisoformat(detection['created_at'].replace('Z', '+00:00'))
                if created_at >= cutoff_date:
                    detections.append(detection)
            else:
                detections.append(detection)

        logger.info(f"Recuperadas {len(detections)} detecções de sabotagem para usuário '{user_id}' (últimos {days} dias).")
        return detections
    except Exception as e:
        logger.error(f"Erro ao recuperar detecções de sabotagem para usuário '{user_id}': {e}", exc_info=True)
        return []
