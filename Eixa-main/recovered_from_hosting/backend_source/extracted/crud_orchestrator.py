...
from eixa_data import (
    ...
    get_all_daily_tasks, get_all_projects, build_agenda_payload,
    ...
    get_unscheduled_task, ensure_recurring_routines
)
...
async def _build_agenda_html_payload(user_id: str) -> Dict[str, Any]:
    ...
    agenda_payload = await build_agenda_payload(user_id)
    return {"agenda": agenda_payload}
...
async def _create_project_micro_task(user_id: str, project_id: str, micro_task_data: Dict[str, Any]) -> Dict[str, Any]:
    project_data = await get_project_data(user_id, project_id)
    if not project_data:
        logger.warning(f"CRUD | MicroTask | Create failed: Project '{project_id}' not found for user '{user_id}'.")
        return {"status": "error", "message": "Projeto não encontrado."}

    description = micro_task_data.get("description")
    if not description:
        return {"status": "error", "message": "Descrição da microtarefa é obrigatória."}

    micro_tasks = project_data.get("micro_tasks") if isinstance(project_data.get("micro_tasks"), list) else []
    new_microtask = {
        "id": micro_task_data.get("id") or str(uuid.uuid4()),
        "description": description.strip(),
        "status": micro_task_data.get("status", "todo"),
        "completed": bool(micro_task_data.get("completed", False)),
        "due_date": micro_task_data.get("due_date"),
        "due_time": micro_task_data.get("due_time"),
        "priority": micro_task_data.get("priority"),
        "duration_minutes": micro_task_data.get("duration_minutes"),
        "energy_level": micro_task_data.get("energy_level"),
        "notes": micro_task_data.get("notes"),
        "created_at": datetime.now(timezone.utc).isoformat(),
        "updated_at": datetime.now(timezone.utc).isoformat()
    }

    micro_tasks.append(new_microtask)
    project_data["micro_tasks"] = micro_tasks
    project_data["updated_at"] = datetime.now(timezone.utc).isoformat()

    try:
        await save_project_data(user_id, project_id, project_data)
        projects_data = await get_all_projects(user_id)
        logger.info(f"CRUD | MicroTask | Created microtask '{new_microtask['id']}' for project '{project_id}' (user '{user_id}').")
        return {
            "status": "success",
            "message": "Microtarefa criada com sucesso.",
            "html_view_data": {"projetos": projects_data}
        }
    except Exception as exc:
        logger.error(f"CRUD | MicroTask | Failed to create microtask for project '{project_id}' and user '{user_id}': {exc}", exc_info=True)
        return {"status": "error", "message": "Não foi possível criar a microtarefa."}


async def _delete_project_micro_task(user_id: str, project_id: str, micro_task_id: str) -> Dict[str, Any]:
    project_data = await get_project_data(user_id, project_id)
    if not project_data:
        logger.warning(f"CRUD | MicroTask | Delete failed: Project '{project_id}' not found for user '{user_id}'.")
        return {"status": "error", "message": "Projeto não encontrado."}

    micro_tasks = project_data.get("micro_tasks") if isinstance(project_data.get("micro_tasks"), list) else []
    filtered_microtasks = [mt for mt in micro_tasks if mt.get("id") != micro_task_id]

    if len(filtered_microtasks) == len(micro_tasks):
        logger.warning(f"CRUD | MicroTask | Delete failed: Microtask '{micro_task_id}' not found in project '{project_id}' (user '{user_id}').")
        return {"status": "error", "message": "Microtarefa não encontrada."}

    project_data["micro_tasks"] = filtered_microtasks
    project_data["updated_at"] = datetime.now(timezone.utc).isoformat()

    try:
        await save_project_data(user_id, project_id, project_data)
        projects_data = await get_all_projects(user_id)
        logger.info(f"CRUD | MicroTask | Deleted microtask '{micro_task_id}' from project '{project_id}' (user '{user_id}').")
        return {
            "status": "success",
            "message": "Microtarefa excluída com sucesso.",
            "html_view_data": {"projetos": projects_data}
        }
    except Exception as exc:
        logger.error(f"CRUD | MicroTask | Failed deleting microtask '{micro_task_id}' from project '{project_id}' (user '{user_id}'): {exc}", exc_info=True)
        return {"status": "error", "message": "Não foi possível excluir a microtarefa."}


async def _update_project_micro_task(user_id: str, project_id: str, micro_task_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    """Atualiza campos de uma microtarefa específica dentro de um projeto."""
    project_data = await get_project_data(user_id, project_id)
    if not project_data:
        logger.warning(f"CRUD | MicroTask | Update failed: Project '{project_id}' not found for user '{user_id}'.")
        return {"status": "error", "message": "Projeto não encontrado."}

    micro_tasks = project_data.get("micro_tasks")
    if not isinstance(micro_tasks, list):
        logger.warning(f"CRUD | MicroTask | Update failed: Project '{project_id}' for user '{user_id}' has no microtasks array.")
        return {"status": "error", "message": "Projeto não possui microtarefas para atualizar."}

    target_microtask = None
    for microtask in micro_tasks:
        if isinstance(microtask, dict) and microtask.get("id") == micro_task_id:
            target_microtask = microtask
            break

    if not target_microtask:
        logger.warning(f"CRUD | MicroTask | Update failed: Microtask '{micro_task_id}' not found in project '{project_id}' for user '{user_id}'.")
        return {"status": "error", "message": "Microtarefa não encontrada."}

    for field, value in updates.items():
        if field == "status":
            target_microtask["status"] = value
            if value == "done":
                target_microtask["completed"] = True
            elif value in {"todo", "in_progress"} and "completed" not in updates:
                target_microtask["completed"] = False
        elif field == "completed":
            target_microtask["completed"] = bool(value)
            if bool(value):
                target_microtask["status"] = target_microtask.get("status", "done") or "done"
            elif target_microtask.get("status") == "done":
                target_microtask["status"] = "todo"
        else:
            target_microtask[field] = value
    ...
    try:
        await save_project_data(user_id, project_id, project_data)
        projects_data = await get_all_projects(user_id)
        logger.info(f"CRUD | MicroTask | Microtask '{micro_task_id}' from project '{project_id}' updated for user '{user_id}'.")
        return {
            "status": "success",
            "message": "Microtarefa atualizada com sucesso.",
            "html_view_data": {"projetos": projects_data}
        }
    except Exception as exc:
        logger.error(
            f"CRUD | MicroTask | Failed saving updated microtask '{micro_task_id}' for project '{project_id}' of user '{user_id}': {exc}",
            exc_info=True
        )
        return {"status": "error", "message": "Não foi possível atualizar a microtarefa."}
...
async def _create_routine(user_id: str, routine_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        stored_template = await get_routine_template(user_id, routine_id)
        routines_for_autofill = [stored_template] if stored_template else None
        await ensure_recurring_routines(user_id, days_ahead=21, routines=routines_for_autofill)
...
async def orchestrate_crud_action(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        elif item_type == 'project_micro_task':
            logger.debug(f"CRUD | orchestrate_crud_action: Project micro task action '{action}' detected.")
            project_id = data.get('project_id')
            if not project_id:
                return {"status": "error", "message": "project_id é obrigatório para microtarefas de projeto.", "data": {}, "debug_info": debug_info}

            if action == 'create':
                return await _create_project_micro_task(user_id, project_id, data)
            elif action == 'update':
                if not item_id:
                    return {"status": "error", "message": "O ID da microtarefa é obrigatório para atualização.", "data": {}, "debug_info": debug_info}
                sanitized_updates = {k: v for k, v in data.items() if k != 'project_id'}
                return await _update_project_micro_task(user_id, project_id, item_id, sanitized_updates)
            elif action == 'delete':
                if not item_id:
                    return {"status": "error", "message": "O ID da microtarefa é obrigatório para exclusão.", "data": {}, "debug_info": debug_info}
                return await _delete_project_micro_task(user_id, project_id, item_id)
            else:
                return {"status": "error", "message": "Ação de microtarefa não reconhecida.", "data": {}, "debug_info": debug_info}
