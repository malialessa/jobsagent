...
CORS(app, resources={
    r"/*": {
        "origins": ["https://eixa.web.app", "https://eixa.firebaseapp.com", "http://localhost:5173", "http://127.0.0.1:5173"],
...
def add_cors_headers_global(response):
    """
    Força a adição de headers CORS em TODAS as respostas.
    Isso garante que mesmo erros ou rotas específicas (como SSE) tenham os headers.
    """
    origin = request.headers.get('Origin')
    allowed_origins = [
        "https://eixa.web.app",
        "https://eixa.firebaseapp.com", 
        "http://localhost:5173",
        "http://127.0.0.1:5173"
    ]
    ...
    if origin in allowed_origins:
        ...
        response.headers['Access-Control-Allow-Origin'] = "https://eixa.web.app"
    ...
    # Para SSE, precisamos de configurações específicas de cache
...
def _build_cors_headers(request_origin: Optional[str] = None) -> dict:
    """Retorna cabeçalhos CORS com base no origin da requisição."""
    ...
    if origin and origin.rstrip('/') in allowed_origins:
        ...
        allow_origin = next(iter(DEFAULT_ALLOWED_ORIGINS), "*")
...
def _initialize_app_globals():
    ...
    if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET or not GOOGLE_REDIRECT_URI or not FRONTEND_URL:
        logger.debug("Google Calendar OAuth não configurado - integração de calendário desabilitada.")
    else:
...
def handle_preflight():
    """Trata requisições OPTIONS (preflight) antes de qualquer outra lógica."""
...
def stream():
    """
    Endpoint SSE simplificado que não depende de Redis.
    Retorna um stream vazio mas válido para manter a conexão.
    """
    def generate():
...
async def google_auth():
    ...
    if google_calendar_utils_instance is None or not google_calendar_utils_instance.oauth_config_ready:
        logger.info("/auth/google: Google Calendar integration not configured - feature disabled.")
        return jsonify({"status": "info", "message": "A integração com Google Calendar não está disponível no momento. Você pode continuar usando todas as outras funcionalidades da EIXA normalmente."}), 200
...
async def interact_api():
    ...
    if not request_json:
        ...
        return jsonify({"status": "error", "response": "Corpo da requisição inválido ou JSON vazio."}), 400
    ...
    if not user_id or not isinstance(user_id, str):
        ...
        return jsonify({"status": "error", "response": "O campo 'user_id' é obrigatório e deve ser uma string."}), 400

    if not GCP_PROJECT:
        logger.critical("GCP_PROJECT não definido. A aplicação não pode operar.")
        return jsonify({"status": "error", "response": "Erro de configuração do servidor (GCP_PROJECT ausente)."}), 500
    ...
    try:
        # Callback para streaming via SSE (desabilitado - sem Redis)
        def publish_chunk(chunk):
            # SSE desabilitado - sem Redis disponível
            pass
        ...
        response_payload = await orchestrate_eixa_response(
            ...
            publish_callback=publish_chunk if request_type == 'chat_and_view' else None
        )
...
async def actions_api():
    ...
    if not item_type or not action:
        return jsonify({"status": "error", "message": "Campos 'item_type' e 'action' são obrigatórios."}), 400
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "Erro interno inesperado."}), 500
...
async def upload_api():
    ...
    if not request_json:
        ...
        return jsonify({"status": "error", "message": "Corpo da requisição inválido ou JSON vazio."}), 400
    ...
    if not user_id or not image_data:
        ...
        return jsonify({"status": "error", "message": "Campos 'user_id' e 'image_data' são obrigatórios."}), 400
