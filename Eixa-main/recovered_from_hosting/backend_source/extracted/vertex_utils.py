...
async def call_gemini_api(
        ...
        try:
            ...
            if stream:
                async def stream_generator():
                    try:
                        # Try using generate_content_async if available
                        ...
                        # Fallback for older vertexai versions or if async method missing
    ...
    api_endpoint = f"{base_url}:streamGenerateContent" if stream else f"{base_url}:generateContent"
    ...
    try:
        # Fix: Criar client fora do generator para streaming
        if stream:
            async def rest_stream_generator():
                # Client criado dentro do generator com seu próprio context manager
                async with httpx.AsyncClient(timeout=120.0) as stream_client:
                    async with stream_client.stream("POST", api_endpoint, headers=headers, json=payload, params={"key": api_key}) as response:
                        ...
                        async for line in response.aiter_lines():
                            if line.strip():
                                try:
                                    ...
                                    pass
            return rest_stream_generator()
        
        # Non-streaming usa client separado
