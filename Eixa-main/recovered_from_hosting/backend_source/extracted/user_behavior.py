...
async def get_user_behavior_data(user_id: str) -> dict: # Função agora assíncrona
    # Fix: get_firestore_document_data já é async, não precisa de to_thread
    data = await get_firestore_document_data('behavior', user_id) 
...
async def save_user_behavior_data(user_id: str, data: dict): # Função agora assíncrona
    # Fix: set_firestore_document já é async, não precisa de to_thread
    await set_firestore_document('behavior', user_id, data)
...
async def track_repetition(user_id: str, current_message: str, history: List[Dict]): # Função agora assíncrona
    ...
    sanitized_current = current_message.strip().lower()
    previous_user_turn = None
    if history:
        # O histórico é retornado em ordem cronológica crescente, então o último item é o mais recente
        previous_user_turn = next((turn for turn in reversed(history) if turn.get('input')), None)

    # Fix: Adiciona await para resolver coroutine corretamente
    user_behavior = await get_user_behavior_data(user_id) # Chama a versão assíncrona

    if previous_user_turn and sanitized_current == previous_user_turn.get('input', '').strip().lower():
    ...
    user_behavior["last_active_timestamp"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
