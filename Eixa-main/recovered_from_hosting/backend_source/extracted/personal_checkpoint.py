...
def _default_performance_meta() -> Dict[str, Any]:
    """Shared default structure for performance metadata."""
    return {
...
def _default_checkpoint_summary(message: str) -> Dict[str, Any]:
    """Creates a placeholder checkpoint payload when there is no history yet."""
...
async def run_weekly_checkpoint(user_id: str):
    ...
    if not self_eval_doc_data:
        ...
        self_eval_doc_data = {
            "user_id": user_id,
            "eixa_performance_meta": _default_performance_meta(),
            "checkpoints": []
        }
...
async def get_latest_self_eval(user_id: str) -> Dict[str, Any]:
    """Returns the full self-eval document plus the latest checkpoint for convenience."""
    self_eval_doc_data = await get_firestore_document_data('self_eval', user_id)

    def _build_response(document: Dict[str, Any] | None, latest_checkpoint: Dict[str, Any]) -> Dict[str, Any]:
        payload = dict(document or {})
        payload.setdefault("user_id", user_id)
        payload.setdefault("checkpoints", [])
        payload.setdefault("eixa_performance_meta", _default_performance_meta())
        payload["latest_checkpoint"] = latest_checkpoint
        return payload

    if not self_eval_doc_data:
        logger.info(f"No self-evaluation document found for user '{user_id}'. Returning default structure with placeholder checkpoint.")
        placeholder = _default_checkpoint_summary("Nenhum resumo de diagnóstico disponível ainda. Interaja mais para que a EIXA possa gerar um.")
        return _build_response({}, placeholder)

    checkpoints = self_eval_doc_data.get("checkpoints", []) or []
    if checkpoints:
        logger.info(f"Retrieved latest self-evaluation for user '{user_id}'.")
        return _build_response(self_eval_doc_data, checkpoints[-1])

    logger.info(f"Self-evaluation document found for user '{user_id}', but no checkpoints recorded yet. Returning default checkpoint payload.")
    placeholder = _default_checkpoint_summary("Nenhum resumo de diagnóstico disponível ainda (sem checkpoints registrados).")
    return _build_response(self_eval_doc_data, placeholder)