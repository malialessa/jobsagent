...
async def analyze_for_nudges(
    ...
    if user_profile:
        ...
        # Nudges baseados em padrões de sabotagem detectados
        from memory_utils import get_recent_sabotage_detections
        try:
            recent_sabotage = await get_recent_sabotage_detections(user_id, days=3, limit=1)
            if recent_sabotage:
                latest_detection = recent_sabotage[0]
                severity = latest_detection.get('severity', 'low')
                total_count = latest_detection.get('total_occurrences', 0)
                top_patterns = latest_detection.get('top_patterns', [])
                
                # Nudge apenas se severidade alta ou média
                if severity in ['high', 'medium'] and total_count >= 5:
                    if top_patterns:
                        top_phrase = top_patterns[0]['phrase']
                        
                        # Nudges contextualizados por padrão
                        if any(p in top_phrase for p in ['procrastin', 'amanhã', 'deixar para depois']):
                            nudges.append(f"⚠️ Percebi um padrão de procrastinação nas suas últimas interações ({total_count} ocorrências). Que tal começar com uma tarefa pequena agora?")
                        elif any(p in top_phrase for p in ['não consigo', 'muito difícil', 'desisto']):
                            nudges.append(f"💪 Notei dificuldades recentes ({total_count}×). Lembre-se: passos pequenos são progresso. Podemos dividir isso em algo menor?")
                        elif any(p in top_phrase for p in ['cansado', 'sem energia', 'sem vontade']):
                            nudges.append(f"😴 Você mencionou cansaço {total_count} vezes recentemente. Que tal uma pausa ou ajustar suas expectativas para hoje?")
                        elif any(p in top_phrase for p in ['perdido', 'bloqueado', 'não sei por onde começar']):
                            nudges.append(f"🧭 Percebi que você está se sentindo perdido(a) ({total_count}×). Vamos mapear um próximo passo concreto juntos?")
                        else:
                            nudges.append(f"🔍 Detectei um padrão recorrente: '{top_phrase}' ({total_count}×). Posso ajudar a quebrar esse ciclo?")
        except Exception as e:
            logger.warning(f"Erro ao verificar sabotage patterns para nudge: {e}")
