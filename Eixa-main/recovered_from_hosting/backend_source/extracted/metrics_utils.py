...
def record_latency(operation: str, duration_ms: float, success: bool, extra: Dict[str, Any] | None = None):
    ...
    if _bq_manager:
        try:
            # Tenta pegar o loop atual, se não existir, não loga (evita RuntimeError)
            try:
                loop = asyncio.get_running_loop()
                asyncio.create_task(_bq_manager.log_operation_metric(operation, duration_ms, success, extra))
            except RuntimeError:
                # Não há event loop rodando, ignora silenciosamente
                logger.debug(f"Metric (no loop) {operation}={duration_ms:.2f}ms success={success}")
